...
export const EmptyState = ({ icon: Icon, title, description, actionLabel, action }: EmptyStateProps) => (
  <div className="rounded-2xl border border-dashed border-slate-300/80 bg-white/70 px-6 py-8 text-center dark:border-white/15 dark:bg-white/[0.03]">
    ...
    {actionLabel ? (
      <button type="button" className="mt-4 rounded-lg border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-100 dark:border-white/15 dark:text-slate-200 dark:hover:bg-white/10">
        {actionLabel}
      </button>
    ) : null}
