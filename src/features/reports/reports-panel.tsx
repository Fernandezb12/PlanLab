...
import { Eye, FileDown, FilePlus2 } from "lucide-react";
...
import { createReportAction, type ReportActionResult } from "@/features/reports/actions";
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="flex gap-1.5">
                    <button
                      type="button"
                      disabled
                      className="inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-500 opacity-70 dark:border-white/10 dark:bg-transparent dark:text-slate-300"
                    >
                      <Eye className="h-4 w-4" />
                      Ver detalle
                    </button>
                    <DocumentExportMenu pdfEndpoint={`/api/export/reports/${report.id}`} tone="primary" />
                  </div>
                </div>
              </Card>
            );
